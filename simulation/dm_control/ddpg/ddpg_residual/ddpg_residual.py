...
if TRAIN_CONTROLLER:
    rewards = []
    avg_rewards = []
    tqdm_range = tqdm(range(NUM_EPISODES_CONTROLLER))
    for episode in tqdm_range:
        ...
        for t in range(DURATION):
            ...
            controller.push(state, action, time_step_2.reward, state_2, -1)
            state = state_2
    ...
    def policy(time_step):
        ...
        t += 1
        state = parse(time_step.observation)
        action = controller.get_action(state, t)
        return action
...
if TRAIN:
    ...
    for episode in tqdm_range:
        ...
        for t in range(DURATION):
            action_a = agent.get_action(state, t=t)
            action_c = controller.get_action(state, t=t)
            action = action_a + action_c
            time_step_2 = env.step(action)
            state_2 = parse(time_step_2.observation)
            agent.push(state, action_a, time_step_2.reward, state_2, -1)
            state = state_2
...
if SHOW_SIMULATION:
    ...
    def policy(time_step):
        ...
        t += 1
        state = parse(time_step.observation)
        action_a = agent.get_action(state, t)
        action_c = controller.get_action(state, t)
        action = action_a + action_c
        return action
